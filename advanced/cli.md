...
为更好地展示 cluster 命令，我们先在本机启动两个节点并组成集群，为避免端口冲突，我们对 emqx2 节点的监听端口做出了调整，例如 MQTT/TCP 监听端口由默认的 1883 改为了 2883，详情请参见 [配置说明](../getting-started/config.md) 与 [配置项]()。
...
::: tip 动作可以由 EMQ X 内置\(称为系统内置动作\)，或者由 EMQ X 插件编写，但不能通过 CLI/API 添加或删除。 :::
...
::: tip 资源类型可以由 EMQ X 内置\(称为系统内置资源类型\)，或者由 EMQ X 插件编写，但不能通过 CLI/API 添加或删除。 :::
...
![image](../.gitbook/assets/rule_metrics%20%281%29.png)
...
![image](../.gitbook/assets/action_metrics%20%281%29.png)
...
![image](../.gitbook/assets/resource_status%20%281%29.png)
