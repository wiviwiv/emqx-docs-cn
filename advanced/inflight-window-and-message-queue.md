...
| 配置项 | 类型 | 可取值 | 默认值 | 说明 |
| :--- | :--- | :--- | :--- | :--- |
| max\_inflight | integer | &gt;= 0 | 32 _\(external\)_,  128 _\(internal\)_ | Inflight Window 长度限制，0 即无限制 |
| max\_mqueue\_len | integer | &gt;= 0 | 1000 _\(external\)_, 10000 _\(internal\)_ | Message Queue 长度限制，0 即无限制 |
| mqueue\_store\_qos0 | enum | `true`, `false` | true | 客户端离线时 EMQ X 是否存储 QoS 0 消息至 Message Queue |
