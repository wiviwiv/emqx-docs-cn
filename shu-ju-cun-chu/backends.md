...
_mqtt:retain_ Hash 存储 Retain 消息:
...
_mqtt\_topic\_msg\_map_ 表定义\(存储主题和消息的映射关系\):
...
某个客户端向主题 test 发布消息后，查询 _mqtt\_acked_ 表:
...
_InfluxDB Backend_ 支持 Hook 与 相应内置函数列表:
...
_Timescale Backend_ 支持 Hook 与 相应内置函数列表:
